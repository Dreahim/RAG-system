...
from models import ProjectModel, DataChunkModel, ResponseSignal
...
async def get_index_project_info(request: Request, project_id: str, search_request: SearchRequest):
    # create project if it doesn't exist
    project_model = await ProjectModel.create_instance(db_client=request.app.db_client)
    ...
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={
            "signal": ResponseSignal.VECTORDB_SEARCH_SUCESS.value,
            "results": results
        }
    )
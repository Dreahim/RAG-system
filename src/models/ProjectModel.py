...
class ProjectModel(BaseDataModel):
    ...
    async def get_all_projecs(self, page: int = 1, page_size: int = 10):
        async with self.db_client() as session:
            async with session.begin():
                query1 = func.count(Project.project_id)
                total_documents = await session.execute(query1)
                total_documents = total_documents.scalar_one()

                total_pages = (total_documents + page_size - 1) // page_size

                query2 = select(Project).offset((page - 1) * page_size ).limit(page_size)
                projects = await session.execute(query2).scalars().all()
